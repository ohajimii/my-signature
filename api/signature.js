// 特殊的 Vercel 配置，告诉它这是一个服务器函数
export const config = {
  runtime: 'edge',
};

// 导出一个默认函数，Vercel 会自动运行它
export default function handler(request) {
  // --- 你签名的核心代码从这里开始 ---

  // 1. 定义动态诗句
  const poeticMoods = {
    morning: "晨曦微露，万物初始", // 5-8
    day: "暖阳正好，时光轻盈",     // 9-16
    dusk: "薄暮冥冥，静候星辰",     // 17-19
    night: "月色如水，梦境将启"      // 20-4
  };
...
<g transform="translate(85, 48)">
  ...
  // 4. 返回这个SVG，并告诉浏览器它是一张SVG图片
  return new Response(svg, {
    headers: {
      'Content-Type': 'image/svg+xml',
      // 添加缓存控制，让浏览器和CDN每60秒更新一次
